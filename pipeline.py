...
class Pipeline():
    def __init__(self,rubric):
        self.keyword_model= pipeline("token-classification",model="yashpwr/resume-ner-bert-v2",aggregation_strategy="simple")    
        self.page_classifier=pipeline(model="facebook/bart-large-mnli")
        self.nlp=spacy.load("en_core_web_sm")
        self.rubric=rubric

    def extract_name_spacy(self,resume_data: list) -> str:
        first_page = resume_data[0].get("page_text", "")
        doc = self.nlp(first_page)
    ...
    def extract_and_tokenize_pii(self,text: str) -> tuple[str, list]:
        doc = self.nlp(text)
        redacted = text
        pii_records = []
        ...
        # LinkedIn
        redacted = re.sub(r'linkedin\.com/in/[^\s]+','LINKEDIN_REDACTED', redacted)

        return redacted, pii_records
    def classify_page(self,page_text:str)->str:
        page_label=self.page_classifier(page_text,candidate_labels=['Tech Skills','Work experience',"other"])
        return page_label['labels'][0]
    def apply_NER(self,page_text: str,rubric:dict) -> List[dict]:
        
        results = self.keyword_model(page_text)
        
        # Add known skills EntityRuler misses
        results = self._apply_entity_ruler(page_text, results,rubric)
        
        return results
    def _apply_entity_ruler(self,page_text: str,existing_entities: list,rubric:dict) -> list:
        already_found = set(e["word"].lower() for e in existing_entities)
        text_lower = page_text.lower()
        new_entities = []
        
        for skill in self.rubric["recruiter_preferred_skills"]:
    ...
    def run_pipe(pdf_path: str) -> tuple[float, dict, str]:
        with mlflow.start_run():    
            ...
            for page_data in resume_data:
                page_data["page_label"] = self.classify_page(page_data["page_text"])
                page_data["page_ner"] = self.apply_NER(page_data["page_text"],rubric)
            print(f"classify+NER: {time.time()-t1:.2f}s")

            # Extract name BEFORE masking
            real_name = self.extract_name_spacy(resume_data)
            ...
            for page in resume_data:
                redacted_text, records = self.extract_and_tokenize_pii(page["page_text"])
